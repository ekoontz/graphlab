...
//    logstream(LOG_INFO) << "release of " << reqid << std::endl;
 ...
 private:
  ...
  size_t deferred_lock_private(const vertex_id_t &vertex,
    ...
    const std::vector<edge_id_t> &inedges =  graph.in_edge_ids(vertex);
    const std::vector<edge_id_t> &outedges = graph.out_edge_ids(vertex);
 ...
 /*   for (size_t i = 1;i < inedges.size(); ++i) {
    ...
    if (rootowner != dc.procid()) {
      if (const_edges) {
        ...
        desc.edgerequests[rootowner] = inedges;
      }
    }
